...
const opcodeToName = instruction => {
...
class CPU {
  ...
  debug() {
    ...
    registers.forEach((name, i) => {
      debugStr += `${name}:\t0x${this.getRegister(name).toString(16).padStart(4, '0')}\t`;
      if (i > 0 && i % 4 === 0) {
        debugStr += '\n';
      }
    });
  ...
  execute(instruction) {
    ...
    try {
      switch (instruction) {
        case instructions.MOV_LIT_REG_PTR.opcode: {
          const value = this.fetch16();
          const reg = this.fetchRegisterIndex();
          const address = this.registers.getUint16(reg);
